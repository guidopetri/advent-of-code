...
for y in range(400):
	for x in range(400):
		distances = [(tup,abs(tup[0]-x)+abs(tup[1]-y)) for tup in content_tuples]
		min_distance = min(distances,key=lambda x:x[1])
		df.iat[x,y] = tuple(tup for tup,distance in distances if distance == min_distance[1])
...
values_to_drop = set(df[0].value_counts().index.tolist()+df[399].value_counts().index.tolist()+df.iloc[0].value_counts().index.tolist()+df.iloc[399].value_counts().index.tolist())

new_df.drop(list(values_to_drop),inplace=True)
new_df.reset_index(drop=False,inplace=True)
new_df.columns = ['index','counts']

totals = new_df.groupby('index').agg({'counts':'sum'}).sort_values(by='counts',ascending=False)
